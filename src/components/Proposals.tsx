...
/* =========================================================
   STATES + DISTRICTS
   Keep this list synchronized with TopNav / Dashboard
========================================================= */
...
export function Proposals({
  ...
  const [proposalState, setProposalState] = useState(
    selectedState !== "All States" && stateDistricts[selectedState]
      ? selectedState
      : "Haryana"
  );
  ...
  const [proposalDistrict, setProposalDistrict] = useState(
    selectedDistrict !== "All Districts" &&
      stateDistricts[selectedState]?.includes(selectedDistrict)
      ? selectedDistrict
      : "All Districts"
  );
  ...
  const handleProposalStateChange = (
    ...
    setProposalDistrict("All Districts");
  };
  ...
  const handleWorkflowAction = async (id: string, action: string) => {
    try {
      const res = await fetch(`/api/v1/projects/${id}/workflow`, {
        ...
        headers: {
          ...
          Authorization: `Bearer ${
            localStorage.getItem("bhumisetu_token") || ""
          }`,
        },
      ...
      if (res.ok) {
        setActionMessage(
          `Project ${id}: ${data.message || "Status updated"}`
        );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        setActionMessage(
          "Proposal submitted successfully to digital scrutiny pipeline."
        );
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case "Approved":
        return (
          <CheckCircle2 className="w-4 h-4 text-cultivated-green" />
        );

      case "Under Scrutiny":
        return <Clock className="w-4 h-4 text-tilled-earth" />;

      case "Rejected":
        return <AlertCircle className="w-4 h-4 text-alluvium-red" />;

      default:
        return <FileText className="w-4 h-4 text-graticule-teal" />;
    }
  };
  ...
  if (view === "create") {
    const availableDistricts =
      stateDistricts[proposalState] || ["All Districts"];
      ...
      <div className="p-8 max-w-4xl mx-auto w-full">
        ...
        <div className="bg-white border border-graticule-teal/30 p-8 shadow-sm">
          ...
          <div className="border-b border-graticule-teal/30 pb-4 mb-6">
            <h2 className="text-2xl font-serif font-semibold text-registry-ink">
              Submit New Project Proposal
            </h2>

            <p className="text-registry-ink/60 text-sm mt-1">
              Initiate a new land acquisition workflow under RFCTLARR Act,
              2013.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="col-span-2">
                <label className="block text-sm font-medium text-registry-ink mb-1">
                  Project Name
                </label>
              ...
              <div>
                ...
                <select
                  ...
                  required
                  value={proposalState}
                  onChange={handleProposalStateChange}
                  className="w-full px-4 py-2 border border-graticule-teal/30 focus:outline-none focus:border-tilled-earth focus:ring-1 focus:ring-tilled-earth bg-white"
                >
              ...
              <div className="col-span-2">
                <label className="block text-sm font-medium text-registry-ink mb-1">
                  Estimated Area Required (Hectares)
                </label>
  ...
  return (
    <div className="p-8 w-full">
      ...
      <div className="flex justify-between items-end mb-8">
        <div>
          <h2 className="text-3xl font-serif font-semibold text-registry-ink">
            Project Proposals
          </h2>

          <p className="text-registry-ink/60 mt-1">
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead>
              <tr className="bg-graticule-teal/10 border-b border-graticule-teal/30 text-registry-ink font-semibold">
                <th className="px-6 py-4 font-mono uppercase tracking-wider text-xs">
                  Reference ID
                </th>
            ...
            <tbody className="divide-y divide-graticule-teal/20">
              {isLoading ? (
                <tr>
                  <td
                    colSpan={8}
                    className="px-6 py-12 text-center text-graticule-teal animate-pulse"
                  >
                    Loading proposals...
                  </td>
                </tr>
                ...
                <tr>
                  <td
                    colSpan={8}
                    className="px-6 py-12 text-center text-registry-ink/60"
                  >
                    No proposals found matching criteria. Create the first one
                    to begin.
                  </td>
                </tr>
              ) : (
                proposals.map((proposal, i) => (
                  ...
                  >
                    {/* ID */}
                    <td className="px-6 py-4 font-mono text-xs text-graticule-teal">
                      {proposal.id}
                    </td>

                    {/* PROJECT */}
                    <td
                      className="px-6 py-4 font-medium text-registry-ink max-w-[250px] truncate"
                    ...
                    {/* MINISTRY */}
                    <td className="px-6 py-4 text-registry-ink/80">
                      {proposal.ministry}
                    </td>
                    ...
                    {/* AREA */}
                    <td className="px-6 py-4 font-mono text-registry-ink/80">
                      {proposal.areaRequired.toFixed(2)}
                    </td>
                    ...
                    <td className="px-6 py-4">
                      {proposal.riskProfile ? (
                        <div
                          ...
                          title={proposal.riskProfile.factors?.join(", ")}
                        >
                          <div
                            className={`w-2 h-2 rounded-full ${
                              proposal.riskProfile.level === "High"
                                ? "bg-alluvium-red"
                                : proposal.riskProfile.level === "Medium"
                                ? "bg-tilled-earth"
                                : "bg-cultivated-green"
                            }`}
                          />
                          ...
                          <span
                            className={`text-xs font-medium px-2 py-0.5 rounded-sm border ${
                              proposal.riskProfile.level === "High"
                                ...
                                : proposal.riskProfile.level === "Medium"
                                ? "bg-tilled-earth/10 text-tilled-earth border-tilled-earth/30"
                                : "bg-cultivated-green/10 text-cultivated-green border-cultivated-green/30"
                            }`}
                          >
                            {proposal.riskProfile.level} (
                            {proposal.riskProfile.score})
                          </span>
                        </div>
                      ) : (
